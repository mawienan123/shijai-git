...
module.exports = async function (targetBranch, options) {
  ...
  //  --quiet 静默执行 不打印任何    使用git fetch --prune origin或git fetch -p 获取被删减后的远程分支，
  ...
  console.info(
    chalk.bgRedBright.black("目标好支") + "：" + chalk.redBright(targetBranch)
  );
  console.info(
    chalk.bgYellowBright.black("稳定好支") +
      "：" +
  ...
  if (isRemoteExist) {
    ...
    const response = await prompts({
      type: "toggle",
      name: "confirm",
      message: `${targetBranch} 的远程分支不存在，是否推送？`,
      initial: true,
      active: "是",
      inactive: "否",
    });
    ...
    if (confirm) {
      await exec(`git push origin  ${targetBranch} --porcelain 1> /dev/null`);
    }
  }
  await exec(`git checkout ${curBranch} >/dev/null 2>&1`);
  signale.success(`合并${isPublish ? "推送" : ""}成功`);
};
